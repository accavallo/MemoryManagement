...
int main(int argc, const char * argv[]) {
    ...
    int **processArray = createProcessesWithPages(), i, j, option;
    printArray(processArray);
    
    while ((option = getopt(argc, (char **)argv, "h")) != -1) {
        switch (option) {
            case 'h':
                printf("Display help\n");
                break;
            case 'c':
                printf("Page replacement algorithm set to clock.\n");
                break;
            case 'f':
                printf("Page replacement algorithm set to first in, first out.\n");
                break;
            case 'l':
                printf("Page replacement algorithm set to least recently used.\n");
                break;
            default:
                break;
        }
    ...
    int faultCount = 0, successCount = 0;
    for (i = 0; i < 10; i++) {
        for (j = 0; j < 8; j++) {
            if (faultOccurred(processArray[i][j], current)) {
                current = replaceClockPage(current, processArray[i][j]);
                faultCount++;
            } else {
                successCount++;
            }
    ...
    getOptimalSolution(processArray);
...
bool faultOccurred(int page, pm_t *frame) {
//    int count = 0;
    pm_t *start = frame;
    while (1) {
...
/* Page fault occurred, now a page needs to be replaced within the frame. */
...
void getOptimalSolution(int **processes) {
